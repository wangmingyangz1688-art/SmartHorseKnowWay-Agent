...
12. 根据群体自动推荐合适的 preferred_activities
13. 如果用户没明确说，根据群体合理推断
        ...
        def search_group(category: str, keywords: List[str]) -> List[dict]:
            ...
            for keyword in keywords:
                ...
                for poi in pois[:5]:
                    ...
                    if candidate["name"] and loc.get("longitude") and loc.get("latitude"):
                        group_results.append(candidate)
                if len(group_results) >= 10:
                    break
            return self._dedupe_candidates(group_results)[:10]
...
要求：
- 如果用户说“看电影/电影/影院”，hard_activity_keywords 必须包含“电影院”或“影城”。
- 如果用户说“烧烤/火锅/咖啡/日料/西餐”，hard_food_keywords 必须包含对应餐饮词。
- 如果用户只是说“拍拍照/出片”，不要写电影院。
- 不要把历史记忆、常见约会偏好写成硬需求。
"""
        try:
            ...
            merged["hard_activity_keywords"] = list(dict.fromkeys([str(item) for item in parsed.get("hard_activity_keywords", []) if item]))
            merged["hard_food_keywords"] = list(dict.fromkeys([str(item) for item in parsed.get("hard_food_keywords", []) if item]))
    ...
    def _create_fast_plan(
        ...
        if extra_venue.get("name") == main_venue.get("name"):
        ...
        plan = {
            ...
            "timeline": [
                ...
                },
                {
                    ...
                    "tags": self._fast_tags("play", parsed_intent, scenario),
                    "booking_available": True,
                    "booking_type": "activity_ticket",
                ...
                },
                {
                    ...
                    "tags": self._fast_tags("eat", parsed_intent, scenario),
                    "booking_available": True,
                    "booking_type": "restaurant",
                ...
                },
                {
                    ...
                    "tags": self._fast_tags("extra", parsed_intent, scenario),
...
"""
    ...
    def _fast_tags(self, activity_type: str, parsed_intent: dict, scenario: dict) -> List[str]:
    ...
    def _build_fast_search_keywords(self, parsed_intent: dict, category: str) -> List[str]:
        ...
        if category == "restaurant":
            if hard_food_keywords:
                ...
                return list(dict.fromkeys(hard_food_keywords))[:2]
            if primary == "solo_healing":
                base = ["一人食", "安静咖啡馆"]
    ...
    def _lock_plan_locations_to_candidates(self, plan: dict, poi_candidates: dict) -> None:
        ...
        for item in plan.get("timeline", []):
            ...
            if not best:
                continue
            item["venue_name"] = best.get("name", item.get("venue_name", ""))
            item["venue_address"] = best.get("address", item.get("venue_address", ""))
            item["venue_location"] = best.get("location", item.get("venue_location"))
            if best.get("id") and not item.get("poi_id"):
                item["poi_id"] = best["id"]
            tags = item.get("tags") if isinstance(item.get("tags"), list) else []
            if "真实POI" not in tags:
                tags.append("真实POI")
            item["tags"] = tags
    ...
    def _lock_agent_plan_to_real_pois(self, plan: dict, request: dict, parsed_intent: dict) -> None:
        ...
        for item in plan.get("timeline", []):
            ...
            if _is_generic_place_name(old_title) or _is_generic_place_name(item.get("title", "")):
    ...
    def _resolve_real_poi_for_plan_item(
        ...
        for keyword in keywords:
            ...
            candidates = [
                ...
                if candidate.get("name") and candidate.get("address") and _has_valid_location(candidate.get("location"))
            ]
            if candidates:
    ...
    def _build_agent_poi_lock_keywords(self, item: dict, parsed_intent: dict) -> List[str]:
        ...
        if activity_type == "eat":
            ...
            keywords.extend(raw_names)
            if any("电影" in x or "影院" in x or "影城" in x for x in hard_activity + raw_names):
    ...
    def _rank_agent_poi_candidates(self, item: dict, candidates: List[dict], parsed_intent: dict) -> dict:
        ...
        hard_activity = [str(x) for x in parsed_intent.get("hard_activity_keywords", []) if x]
        hard_food = [str(x) for x in parsed_intent.get("hard_food_keywords", []) if x]
        target_keywords = hard_food if item.get("activity_type") == "eat" else hard_activity

        def score(candidate: dict) -> int:
            text = f"{candidate.get('name', '')} {candidate.get('type', '')} {candidate.get('address', '')} {candidate.get('keyword', '')}"
            value = 0
            ...
            if item.get("activity_type") == "eat" and any(word in text for word in ["餐厅", "美食", "烧烤", "火锅", "料理", "咖啡"]):
                value += 8
            if item.get("activity_type") != "eat" and any(word in text for word in ["影院", "影城", "电影", "公园", "商场", "展览", "景区"]):
                value += 8
    ...
    def _build_fallback_action(self, action: dict, plan: dict, result: dict) -> Optional[dict]:
        ...
        if action_type == "book_activity":
            ...
            ticket_count = params.get("ticket_count", 1)
            fallback_venue = self._pick_free_activity_name(plan, venue_name)
            fallback_action = {
                "action_id": f"{action.get('action_id', 'act')}_fallback_free",
                "action_type": "book_activity",
                "description": f"改为免预约活动：{fallback_venue} x{ticket_count}",
                "params": {
                    **params,
            ...
            return {
                "action": fallback_action,
                "reason": f"{venue_name}可能无票或预约名额不足，已准备附近免费/免预约活动作为替代。",
            }
        ...
        if action_type == "order_delivery":
            fallback_action = {
                **action,
                ...
                "description": f"重试{action.get('description', '配送下单')}",
            }
            return {
                "action": fallback_action,
        ...
        return {
            "action": {
                **action,
                "action_id": f"{action.get('action_id', 'act')}_retry",
                "description": f"重试{action.get('description', '该动作')}",
            },
            "reason": "该动作失败后可以重试一次。",
        }
        ...
        with ThreadPoolExecutor(max_workers=min(3, len(tasks) or 1)) as executor:
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    all_results.append(f"【{kw}搜索结果】搜索失败")
